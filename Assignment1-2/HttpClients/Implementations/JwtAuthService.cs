...
public class JwtAuthService : IAuthService
{
    ...
    public async Task LoginAsync(string username, string password)
    {
        ...
        HttpResponseMessage response = await client.PostAsync("https://localhost:7140/auth/login", content);
    ...
    public async Task RegisterAsync(UserCreationDto dto)
    {
        ...
        StringContent content = new(userAsJson, Encoding.UTF8, "application/json");
        HttpResponseMessage response = await client.PostAsync("https://localhost:7140/auth/register", content);
        string responseContent = await response.Content.ReadAsStringAsync();
